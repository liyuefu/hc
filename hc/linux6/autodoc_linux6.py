...
update: 2023.03.01 read config.ini, save check history date, dmp, rman, datafile data in config.ini
update: 2023.04.13 .修复了文件名没有包括数据库名的错误
"""
...
logging.basicConfig(level=logging.DEBUG,format='%(asctime)s %(name)-12s %(levelname)-8s %(message)s',datefmt='%m-%d %H:%M',filename='/tmp/autodoc_linux6.log',filemode='w')
logging.info('starting autodoc_linux6.py now...')
...
filename_end=client_app_db_name + ur"健康检查报告"
#date of today
...
#head page
body.append(paragraph(client_name))
body.append(paragraph(client_app_name))
body.append(paragraph(filename_end))
emptyline=20
while emptyline>0:
...
record=[[ur"日期",ur"作     者",ur"版本",ur"修 改 记 录"],[today,lidao_engineer_name,ur"V1.0",ur"初始创建"]]
...
record=[[ur"拷贝编号",ur" 姓    名",ur"单             位"],["1",client_dba_name,client_name],["2",ur"利道技术部",ur"利道软件"]]
body.append(table(record,twunit='dxa',tblw=9000,borders={'all':{"sz": 10, "val": "single", "color": "#000000", "space": "0"}}))
